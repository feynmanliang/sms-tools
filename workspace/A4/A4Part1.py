...
def extractMainLobe(window, M):
    ...
    ### Your code here
    W = 20*np.log10(np.abs(fftshift(fft(w, 8*M))) + 10e-16)
    lmin_idx = int(math.floor(len(W)/2))
    while lmin_idx < len(W) and W[lmin_idx] >= W[lmin_idx+1]:
        lmin_idx += 1
    rmin_idx = int(math.floor((len(W)+1)/2))
    while rmin_idx > 0 and W[rmin_idx] >= W[rmin_idx-1]:
        rmin_idx -= 1
    return W[rmin_idx:lmin_idx+1]
